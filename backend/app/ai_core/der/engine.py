...
def _load_via_ffmpeg_then_scipy(filepath, frame_offset=0, num_frames=-1, normalize=False, channels_first=True):
    """Last-resort fallback: use ffmpeg to convert any audio to PCM WAV, then load via scipy."""
    import shutil
    import subprocess
    import tempfile
    ...
    if ffmpeg is None:
        raise RuntimeError("ffmpeg not found on PATH — cannot convert non-wav enrollment audio")
    ...
    try:
        result = subprocess.run(
            [ffmpeg, "-y", "-i", str(filepath), "-ac", "1", "-ar", "16000", "-acodec", "pcm_s16le", tmp_path],
            capture_output=True, text=True, timeout=60,
        )
        if result.returncode != 0:
            raise RuntimeError(f"ffmpeg conversion failed (rc={result.returncode}): {result.stderr[:300]}")
...
def _safe_audio_load(filepath, frame_offset=0, num_frames=-1, normalize=False, channels_first=True, format=None):
    ...
    print(f"  [_safe_audio_load] Loading: {filepath} (ext={ext})")

    if ext == ".wav":
        try:
            result = _load_wav_via_scipy(
                filepath,
                frame_offset=frame_offset,
                num_frames=num_frames,
                normalize=normalize,
            ...
            print(f"  [_safe_audio_load] scipy OK for {filepath}")
            return result
        except Exception as e_scipy:
            print(f"  [_safe_audio_load] scipy failed for {filepath}: {e_scipy}")
            pass

    # For non-wav files, try ffmpeg FIRST (more reliable on Windows)
    if ext != ".wav":
        print(f"  [_safe_audio_load] Non-wav file, trying ffmpeg first for {filepath}")
        try:
            result = _load_via_ffmpeg_then_scipy(
                filepath,
                frame_offset=frame_offset,
                num_frames=num_frames,
                normalize=normalize,
            ...
            print(f"  [_safe_audio_load] ffmpeg OK for {filepath}")
            return result
        except Exception as ffmpeg_err:
            print(f"  [_safe_audio_load] ffmpeg fallback failed for {filepath}: {ffmpeg_err}")

    try:
        result = _orig_torchaudio_load(
            filepath,
            frame_offset=frame_offset,
            num_frames=num_frames,
            normalize=normalize,
        ...
        print(f"  [_safe_audio_load] torchaudio OK for {filepath}")
        return result
    except (RuntimeError, OSError, Exception) as e:
        print(f"  [_safe_audio_load] torchaudio FAILED for {filepath}: {type(e).__name__}: {str(e)[:200]}")
...
def build_enrollment_centroids(enrollment_dir, ecapa, resampler, max_speakers=None):
    ...
    centroids = []
    names = []
    print(f"  [ENROLLMENT] Processing {len(groups)} speaker groups")
    for spk_name, files in groups:
        print(f"  [ENROLLMENT] Speaker '{spk_name}': {len(files)} files")
        speaker_items = []
        for fpath in files:
            print(f"  [ENROLLMENT]   Loading: {fpath}")
            try:
                wav = _load_audio_mono16k(fpath)
                print(f"  [ENROLLMENT]   Loaded OK: shape={wav.shape}")
            except Exception as load_err:
                print(f"  [ENROLLMENT]   LOAD FAILED: {type(load_err).__name__}: {str(load_err)[:200]}")
                stats["files_skipped"] += 1
                continue
            dur_sec = float(wav.shape[1] / TARGET_SR)
            if dur_sec < ENROLLMENT_MIN_AUDIO_SEC:
                print(f"  [ENROLLMENT]   SKIPPED: too short ({dur_sec:.1f}s < {ENROLLMENT_MIN_AUDIO_SEC}s)")
                stats["files_skipped"] += 1
                continue
            emb, used_dur, _ = get_segment_embedding(ecapa, wav, resampler, 0.0, dur_sec)
            if emb is None or used_dur < 0.2:
                print(f"  [ENROLLMENT]   SKIPPED: bad embedding (emb={emb is not None}, dur={used_dur:.2f})")
                stats["files_skipped"] += 1
                continue
            speaker_items.append((emb.astype(np.float32), float(used_dur)))
            stats["files_used"] += 1
            print(f"  [ENROLLMENT]   OK: emb dim={emb.shape[0]}, dur={used_dur:.1f}s")

        if not speaker_items:
            print(f"  [ENROLLMENT] Speaker '{spk_name}': NO valid items")
            continue

        dim = speaker_items[0][0].shape[0]
        centroid = _weighted_centroid(speaker_items, dim)
        if centroid is None:
            print(f"  [ENROLLMENT] Speaker '{spk_name}': centroid computation failed")
            continue
        centroids.append(centroid.astype(np.float32))
        names.append(spk_name)
        print(f"  [ENROLLMENT] Speaker '{spk_name}': centroid OK")

    print(f"  [ENROLLMENT] Final: {len(centroids)} centroids for {names}")
    if not centroids:
        return None, [], stats
