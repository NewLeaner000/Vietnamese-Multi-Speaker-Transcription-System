...
class PipelineConfig:
    ...
    prefer_rule_normalize: bool = False
    use_rule_reducer: bool = False
...
class QwenRunner:
    def __init__(self, cfg: PipelineConfig):
        ...
        self.model = Llama(
            ...
            n_gpu_layers=-1,
            n_ctx=cfg.max_input_tokens + 512,
            verbose=False,
        )
        self.load_time_sec = time.perf_counter() - load_t0
        print(f"[QWEN-PROFILE] model_load_sec={self.load_time_sec:.2f}", flush=True)
    ...
    def generate_json(self, system_prompt: str, user_prompt: str, max_new_tokens: int) -> Dict[str, Any]:
        ...
        response = self.model.create_chat_completion(
            ...
            temperature=self.cfg.temperature if self.cfg.do_sample else 0.1,
            repeat_penalty=self.cfg.repetition_penalty,
        )
        
        elapsed = time.perf_counter() - t0
        print(f"[QWEN-PROFILE] generate_sec={elapsed:.3f} | max_new_tokens={max_new_tokens}", flush=True)
...
def _cfg_for_mode(base_cfg: PipelineConfig, qwen_mode: str) -> PipelineConfig:
    ...
    if qwen_mode == "fast":
        ...
        cfg.reducer_max_new_tokens = max(cfg.reducer_max_new_tokens, 512)
        cfg.use_rule_reducer = False
...
def _cfg_for_model(base_cfg: PipelineConfig, model_path: str) -> PipelineConfig:
    cfg = replace(base_cfg)
...
def run_pipeline(args: argparse.Namespace) -> Dict[str, Any]:
    normalize_model_path = args.normalize_model_path or args.model_path
    summary_model_path = args.summary_model_path or normalize_model_path
    ...
    def get_runner(model_path: str) -> QwenRunner:
        if model_path not in runner_cache:
            print(f"[3/7] Loading model lazily: {model_path}", flush=True)
            t_load = time.perf_counter()
            runner_cache[model_path] = QwenRunner(_cfg_for_model(cfg, model_path))
            runner_load_times[model_path] = round(time.perf_counter() - t_load, 4)
        return runner_cache[model_path]

    if need_llm:
        print(
            f"[3/7] Qwen mode={args.qwen_mode} | normalize_model={normalize_model_path} | "
            f"summary_model={summary_model_path} | lazy loading enabled",
            flush=True,
        )
    else:
    ...
    for i, block in enumerate(blocks):
        ...
        runner = get_runner(normalize_model_path) if use_llm else None
    ...
    for i, nb in enumerate(normalized_blocks):
        ...
        runner = get_runner(summary_model_path) if use_llm else None
        mode = "llm" if use_llm else "rule"
    ...
    if need_llm and (not cfg.use_rule_reducer) and cfg.reducer_max_new_tokens > 0:
        reducer_runner = get_runner(summary_model_path)
    ...
    perf["normalize_model_path"] = normalize_model_path
    perf["summary_model_path"] = summary_model_path
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--model_path", type=str, default="core/qwen/checkpoints/Qwen2.5-7B-Instruct-Q4_K_M.gguf", help="Backward compatible single-model option")
    ap.add_argument("--normalize_model_path", type=str, default=None, help="Model used for normalization stage")
    ap.add_argument("--summary_model_path", type=str, default=None, help="Model used for summary/reducer stage")
