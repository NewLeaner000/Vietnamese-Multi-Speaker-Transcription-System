...
def infer_speaker_mode_from_lines(lines_local: List[str], seg_type: str) -> str:
    ...
    question_hits += len(re.findall(r"\b(ủa|vậy|nhưng mà|sao|thế|bao lâu|có nghĩa là|đúng không|phải không)\b", text))
    opinion_hits = len(re.findall(r"\b(em nghĩ|theo em|anh nghĩ|chị nghĩ|ý là|có nghĩa là|em thấy|anh thấy|chị thấy)\b", text))
...
def build_compact_speaker_summary(seg_type: str, mode: str, lines_local: List[str]) -> str:
    ...
    if mode == 'host':
        return 'Chủ yếu dẫn dắt chương trình hoặc chuyển cảnh.'
    if mode == 'intro':
        return 'Chủ yếu giới thiệu khách mời hoặc mở chủ đề.'
    if mode == 'interaction':
        return f'Chủ yếu tham gia tương tác ngắn về {topic}.'
    if mode == 'question':
        return f'Chủ yếu đặt câu hỏi hoặc gợi mở về {topic}.'
    if mode == 'opinion':
        return f'Chủ yếu nêu quan điểm hoặc góp ý về {topic}.'
    if mode == 'share':
...
def build_best_speaker_main_summary(best_item: Dict[str, Any], items: List[Dict[str, Any]], speaker_lines: Optional[List[str]] = None) -> str:
...
def build_conversation_main_summary(groups: List[Dict[str, Any]], meeting_type: str) -> str:
    ...
    secondary = None
    for t in seg_types:
        if t != main_type and t != 'transition':
            secondary = t
            break
    ...
    if meeting_type == 'talkshow':
        ...
        if any(str(g.get('_seg_type')) == 'guest_intro' for g in groups):
            parts.append('Cuộc hội thoại mở đầu bằng phần giới thiệu khách mời.')
        main_topic = topic_phrase_from_seg_type(main_type, joined)
        parts.append(f'Nội dung chính tập trung vào {main_topic}.')
        if secondary and secondary not in ('guest_intro', 'transition', main_type):
            sec_topic = topic_phrase_from_seg_type(secondary, joined)
            if sec_topic != main_topic:
                parts.append(f'Ngoài ra còn nhắc đến {sec_topic}.')
        return ' '.join(parts[:3]).strip()

    main_topic = topic_phrase_from_seg_type(main_type, joined)
    if secondary and secondary not in ('transition', main_type):
        sec_topic = topic_phrase_from_seg_type(secondary, joined)
        if sec_topic != main_topic:
...
def reduce_summaries_rule(block_summaries: List[Dict[str, Any]], normalized_blocks: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    for speaker, items in sorted(per_speaker.items()):
        ...
        main_summary = build_best_speaker_main_summary(best_item, items, speaker_lines)
    ...
    return {
        ...
        "quality_notes": ["rule_reducer_v6_compact_main_summaries"],
    }
