...
def build_rule_speaker_main_summary(seg_types: List[str]) -> str:
    ...
    main = uniq[0]
    main_phrase_map = {
        'transition': 'dẫn dắt chương trình hoặc chuyển cảnh',
        'game': 'trò chơi hoặc tương tác nhẹ',
        'guest_intro': 'giới thiệu khách mời hoặc mở chủ đề',
        'family_story': 'câu chuyện gia đình, phân chia trách nhiệm hoặc việc dạy con',
        'health_story': 'sức khỏe, áp lực, ăn uống hoặc giảm cân',
        'work_story': 'công việc, khách hàng hoặc tình huống nghề nghiệp',
        'discussion': 'nội dung trao đổi chính',
        'other': 'một phần trao đổi khác',
    }
    main_phrase = main_phrase_map.get(main, 'nội dung trao đổi chính')
    if main == 'transition':
        return "Chủ yếu dẫn dắt chương trình hoặc chuyển cảnh."
    return f"Chủ yếu trao đổi về {main_phrase}."
...
def infer_speaker_mode_from_lines(lines_local: List[str], seg_type: str) -> str:
    ...
    host_hits = len(re.findall(r"\b(quý vị|chúng tôi sẽ quay trở lại|xin mời quý vị|chào mừng quý vị|chương trình|nghỉ giải lao|quay trở lại)\b", text))
    intro_hits = len(re.findall(r"\b(khách mời|xin mời|giới thiệu|câu chuyện ngày hôm nay|mở đầu)\b", text))
    question_hits = text.count('?')
    question_hits += len(re.findall(r"\b(ủa|vậy|nhưng mà|sao|thế|bao lâu|có nghĩa là|đúng không|phải không|là sao|thế nào|ai dạy|vì sao|tại sao|hả)\b", text))
    opinion_hits = len(re.findall(r"\b(em nghĩ|theo em|anh nghĩ|chị nghĩ|ý là|có nghĩa là|em thấy|anh thấy|chị thấy|quan trọng là)\b", text))
    share_hits = len(re.findall(r"\b(tôi|em|anh|chị)\b", text))
    narrative_hits = len(re.findall(r"\b(hồi đó|trước đây|sau đó|một ngày|đến năm|tại vì|rồi|nhưng mà)\b", text))

    if host_hits >= 2:
        return 'host'
    if intro_hits >= 2 and intro_hits >= question_hits:
        return 'intro'
    if seg_type == 'game' and question_hits <= 1:
        return 'interaction'
    if share_hits >= 6 and narrative_hits >= 3 and question_hits <= 3:
        return 'share'
    if opinion_hits >= 2 and question_hits <= opinion_hits + 1:
        return 'opinion'
    if question_hits >= max(2, opinion_hits + 1):
        return 'question'
    if seg_type == 'family_story' and narrative_hits >= 2:
        return 'share'
    return 'discussion'
...
def topic_phrase_from_seg_type(seg_type: str, joined_text: str = '') -> str:
    ...
    if seg_type == 'family_story':
        ...
        if any(k in low for k in ['tiền', 'chi phí', 'điện nước', 'chia đôi']):
            topic_parts.append('cách quản lý chi phí trong gia đình')
        if any(k in low for k in ['khiêu vũ', 'trung tâm']):
            topic_parts.append('câu chuyện công việc và cuộc sống gia đình')
        if not topic_parts:
            topic_parts.append('đời sống gia đình và phân chia trách nhiệm')
...
def build_compact_speaker_summary(seg_type: str, mode: str, lines_local: List[str]) -> str:
    ...
    if mode == 'host':
        return 'Chủ yếu dẫn dắt chương trình hoặc chuyển cảnh.'
    if mode == 'intro':
        return f'Chủ yếu giới thiệu khách mời hoặc mở chủ đề về {topic}.' if seg_type not in ('guest_intro', 'transition') else 'Chủ yếu giới thiệu khách mời hoặc mở chủ đề.'
    if mode == 'interaction':
        return f'Chủ yếu tương tác ngắn về {topic}.'
    if mode == 'question':
        return f'Chủ yếu đặt câu hỏi hoặc gợi mở về {topic}.'
    if mode == 'opinion':
        return f'Chủ yếu nêu quan điểm về {topic}.'
    if mode == 'share':
        return f'Chủ yếu chia sẻ về {topic}.'
    return f'Chủ yếu trao đổi về {topic}.'
...
def build_best_speaker_main_summary(best_item: Dict[str, Any], items: List[Dict[str, Any]], speaker_lines: Optional[List[str]] = None, extra_lines: Optional[List[str]] = None) -> str:
    ...
    lines_local = []
    if speaker_lines:
        lines_local.extend(speaker_lines)
    if extra_lines:
        for x in extra_lines:
            x = normalize_whitespace(x)
            if x and x not in lines_local:
                lines_local.append(x)
    lines_local = lines_local[:8]
    mode = infer_speaker_mode_from_lines(lines_local, seg_type)
    return cleanup_repeated_sentences(build_compact_speaker_summary(seg_type, mode, lines_local))
...
def reduce_summaries_rule(block_summaries: List[Dict[str, Any]], normalized_blocks: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "quality_notes": ["rule_reducer_v7_compact_main_summaries_refined"],
    }
