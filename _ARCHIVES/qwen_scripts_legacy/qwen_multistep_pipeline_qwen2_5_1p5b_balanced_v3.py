...
def reduce_summaries_rule(block_summaries: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for speaker, items in sorted(per_speaker.items()):
        ...
        for item in items:
            ...
            if insight:
                valid_count += 1
                if insight not in merged_texts:
                    merged_texts.append(insight)
            for span in item.get("evidence_spans", []):
                if isinstance(span, list) and len(span) == 2:
                    spans.append(span)
        if not merged_texts:
            merged_texts = ["Có nhiều lượt phát biểu trong đoạn này." if len(items) >= 2 else "Tham gia trao đổi trong đoạn này."]
        final_insights.append({
            "speaker": speaker,
            "insight": cleanup_repeated_sentences(" ".join(merged_texts[:2])),
    ...
    return {
        ...
        "quality_notes": ["rule_reducer_v3_sanitized"],
    }
