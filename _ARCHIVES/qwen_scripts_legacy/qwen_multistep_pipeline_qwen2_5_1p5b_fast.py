...
class PipelineConfig:
    ...
    normalize_max_new_tokens: int = 512
    local_summary_max_new_tokens: int = 420
...
def should_skip_llm_summary(normalized_block: Dict[str, Any], cfg: PipelineConfig) -> bool:
    ...
    if is_transition and (len(lines) <= 4 or total_chars <= 500):
        return True
    return False
...
def summarize_block_rule(normalized_block: Dict[str, Any]) -> Dict[str, Any]:
    ...
    text_blob = " ".join(x.get("clean_text", "") for x in lines).lower()
    if normalized_block.get("is_transition_heavy", False):
        segment_type = "transition"
        overview = "Đoạn chuyển tiếp / giới thiệu / nghỉ giải lao của chương trình."
    elif "trò chơi" in text_blob or "game" in text_blob or "calo" in text_blob:
        segment_type = "game"
        overview = "Đoạn trò chơi hoặc tương tác nhẹ giữa các người nói."
    elif "xin mời" in text_blob or "khách mời" in text_blob:
        segment_type = "guest_intro"
        overview = "Đoạn giới thiệu khách mời hoặc mở đầu chủ đề mới."
    else:
        segment_type = "other"
        overview = "Đoạn trao đổi ngắn; không đủ bằng chứng để tóm tắt sâu hơn theo rule-based."
    ...
    for spk in speakers[:4]:
        ...
        insights.append({
            "speaker": spk,
            "insight": "Tham gia vào đoạn trao đổi này.",
            "evidence_spans": [[first["start"], spk_lines[-1]["end"]]],
        })
...
def summarize_block_llm(runner: QwenRunner, normalized_block: Dict[str, Any], cfg: PipelineConfig) -> Dict[str, Any]:
    ...
    result.setdefault("segment_type", "other")
...
def reduce_summaries_rule(block_summaries: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for bs in block_summaries:
        bid = bs.get("block_id")
        seg_type = bs.get("segment_type", "other")
        overview = normalize_whitespace(bs.get("overview", ""))
        if overview:
            overviews.append(overview)
        title = {
            "discussion": "Discussion Segment",
            "transition": "Transition Segment",
            "game": "Game Segment",
            "guest_intro": "Guest Introduction Segment",
            "other": "Other Segment",
        }.get(seg_type, "Other Segment")
        segments.append({
            "title": title,
            "block_ids": [bid],
            "summary": overview or "Không đủ bằng chứng để tóm tắt rõ hơn.",
        })
        ...
        for item in bs.get("speaker_insights", []):
    ...
    for speaker, items in sorted(per_speaker.items()):
        ...
        spans = []
        for item in items[:3]:
            insight = normalize_whitespace(item.get("insight", ""))
        ...
        final_insights.append({
            "speaker": speaker,
            "insight": " ".join(merged_texts[:2]) if merged_texts else "Tham gia cuộc trao đổi.",
            "evidence_spans": spans[:4],
        })

    non_transition = [s for s in block_summaries if s.get("segment_type") != "transition"]
    distinct_types = {s.get("segment_type", "other") for s in non_transition}
    ...
    if not overviews:
        ...
        meeting_overview = " ".join(overviews[:3])

    if any(s.get("segment_type") == "guest_intro" for s in block_summaries):
        meeting_type = "talkshow"
    elif any(s.get("segment_type") == "game" for s in block_summaries):
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--normalize_max_new_tokens", type=int, default=512)
    ap.add_argument("--local_summary_max_new_tokens", type=int, default=420)
